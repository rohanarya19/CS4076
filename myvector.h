...
template<class T> myVector<T>::myVector(int i)
{
    maxsize=i;
    array= new T[maxsize];
    vsize=0;
...
template<class T> void myVector<T>::push_back(const T& i)
{
    if(vsize+1>maxsize)
    {
        alloc_new();
...
template<class T> T myVector<T>::operator[](int i)
{
    return array[i];
}
...
template<class T> myVector<T>& myVector<T>::operator+=(const T& i)
{
    this->push_back(i);
    return *this;
}

template<class T> int myVector<T>::at(T& i)
{
    if(i<vsize)
    {
        return array[i];
...
template<class T> void myVector<T>::alloc_new()
{
    maxsize=vsize*2;
    T* tmp=new T[maxsize];
    int i=0;
